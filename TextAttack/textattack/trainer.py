...
import pandas as pd
#|.*output.dense.weight
#|.*intermediate.dense.weight|.*pooler.dense.weight
UNIREGEX=r"(.*self.*weight|.*attention.output.dense.weight|.*uni_linear.*.weight)"
...
class Trainer:
    ...
    def get_optimizer_and_scheduler(self, model, num_training_steps):
        ...
        if isinstance(model, transformers.PreTrainedModel):
            ...
            print(f"\n##### Optimizer Version: {self.training_args.ver} #####\n")
            param_optimizer, param_cayley = [], []
            for name, param in model.named_parameters():
                # this applies to both pen_cay, emb_cay, and cay
                if "cay" in self.training_args.ver and re.match(UNIREGEX, name):
                    param_cayley += [(name, param)]
                else:
                    param_optimizer += [(name, param)]
            no_decay = ["bias", "LayerNorm.bias", "LayerNorm.weight"]
            optimizer_grouped_parameters = [
                {
                    "params": [
                        p
                        for n, p in param_cayley
                    ],
    ...
    def training_step(self, model, tokenizer, batch):
        ...
        if isinstance(model, transformers.PreTrainedModel) or (
            ...
            # df = pd.DataFrame.from_dict(vocab, orient='index',columns=['id'])
            # df.index.name = 'token'
            # df.set_index('id',inplace=True)
            # print(df)
            # with open('/home/h/dev/pathfind/uniBERT/vocab_bert.txt') as fp:
            #     lines = fp.readlines()
            #     lines = [line.replace('\n','') for line in lines]
            # df_vocab_bert = pd.DataFrame(lines,columns=['word'])
            # df_vocab_bert['index'] = df_vocab_bert.index
            ## NOTE mod end
        ...
        if self.task_type == "regression":
            ...
            ## NOTE mod start
            if 'svm' in self.training_args.ver:
                self.loss_fct = torch.nn.MultiMarginLoss(margin=self.training_args.margin) 
            ## NOTE mod end
        ...
        ## NOTE mod start
        if 'pen' in self.training_args.ver:
            w = model.bert.embeddings.word_embeddings_syno.weight
            distance = w @ w.T 
            loss_weight = torch.linalg.norm(distance - model.bert.embeddings.syno_dis)
            # print(f"original loss: {loss}, weight loss: {loss_weight}")
            loss += self.training_args.penalty*loss_weight

        ## semi unitary penality on the classifier
        # if 'cay' in self.training_args.ver:
        #     w = model.classifier.weight
        #     prod = w.T @ w
        #     error = torch.linalg.norm(prod-torch.eye(prod.shape[0],device=prod.device))
        #     loss += 1000.0*error
    ...
    def train(self):
        ...
        ## NOTE mod start
        model.config.margin = self.training_args.margin
        model.config.penalty = self.training_args.penalty
        model.config.ver = self.training_args.ver
        # print(model)
        ...
        for epoch in range(1, self.training_args.num_epochs + 1):
            ...
            if self.training_args.log_to_tb:
                ...
                #         loss_weight = self.args.penalty*distance
                #         self._tb_log({f"eval/loss_weight": loss_weight.item()})
                #         self._tb_log({f"eval/loss_total": eval_score+loss_weight.item()})
                #         self._tb_log({f"eval/distance": distance.item()})
                #         self._tb_log({f"eval/penalty": self.args.penalty})
            ## NOTE mod end
            if self.training_args.log_to_wandb:
                self._wandb_log(
    ...
    ## NOTE mod start
        self._record_unitarity()
        self._save_weight_loss()
    def _save_weight_loss(self):
        output_dir=self.training_args.output_dir
        model=self.model_wrapper.model
        with open(output_dir+'/weight_loss.txt','w') as f:
            try:
                w = model.bert.embeddings.word_embeddings_syno.weight
                distance = w @ w.T 
                loss_weight = self.args.penalty*torch.linalg.norm(distance - model.bert.embeddings.syno_dis)
                f.write(f"loss weight: {loss_weight:.3f}, self.args.penalty: {self.args.penalty:.5f}\n")
            except: 
                f.write(f"loss weight: 0, self.args.penalty: 0\n")
    def _record_unitarity(self):
        # print unitarity
        def error(layer):
            w = layer.data
            e = torch.abs(w @ w.T - torch.eye(w.shape[0],device=w.device)).sum().sum()/(w.shape[0]*w.shape[1])
            return e
        filename = self.training_args.output_dir+'/unitarity.txt'
        with open(filename,'w') as f:
            f.write("*** Model Layers & Unitariry ***\n\n")
            for name, param in self.model_wrapper.model.named_parameters():
                if re.match(UNIREGEX, name):
                    f.write(f"{error(param):.4f} {name}\n")
                else:
                    f.write(f"------ {name}\n")
        logger.info(f"Wrote unitarity to {filename}")
    ## NOTE mod end
    def evaluate(self):
