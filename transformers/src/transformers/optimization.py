    ...
    def step(self, closure: Callable = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                ## NOTE mod start
                # if group['stiefel'] and random.randrange(1) == 0:
                if group['stiefel']:
                    # if parameter is a wide matrix, break it down to smaller unitary matrices
                    # h,w = p.data.shape
                    # # print(f"{h},{w}")
                    # if w > h:
                    #     assert w % h == 0
                    #     for i in range(w//h):
                    #         # print(p.data[:,h*i:h*(i+1)])
                    #         q, r = torch.linalg.qr(p.data[:,h*i:h*(i+1)])
                    #         unitary = q*torch.diag(r).sign()
                    #         p.data[:,h*i:h*(i+1)] = unitary
                    # else:
                    q, r = torch.linalg.qr(p.data)
                    unitary = q*torch.diag(r).sign()
                    p.data.copy_(unitary)
                ## NOTE mod end
