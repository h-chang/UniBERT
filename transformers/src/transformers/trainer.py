...
## NOTE mod start
# |.*output.dense.weight
UNIREGEX=r"(.*self.*weight|.*attention.output.dense.weight|.*intermediate.dense.weight|.*pooler.dense.weight|.*uni_linear.*.weight)"
## NOTE mod end
import collections
import inspect
import math
...
class Trainer:
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            print(f"\n##### Optimizer Version: {self.args.ver}, unitary constraint on the following #####\n")
            # seperate model into ones that need unitarity constraints and no constraints
            param_optimizer, param_cayley = [], []
            for name, param in self.model.named_parameters():
                if "cay" in self.args.ver and re.match(UNIREGEX, name):
                    param_cayley += [(name, param)]
                else:
                    param_optimizer += [(name, param)]
            for name, param in param_cayley:
                print(name)
            print("")
            # further speperate the no constraints into with weight decay or no decay
    ...
    ## NOTE mod start
        # save embedding penality and unitarity
        if self.args.should_save:
            self._save_weight_loss(model,output_dir)
            self._save_unitary_error(model,output_dir)
        
    def _save_weight_loss(self,model,output_dir):
    ...
    def _save_unitary_error(self,model,output_dir):
        # calculate the distance to identity funciton
        def error(layer):
            w = layer.data
            e = torch.abs(w.T @ w - torch.eye(w.shape[1],device=w.device)).sum().sum()/(w.shape[0]*w.shape[1])
            return e
        # go through all weights
        with open(output_dir+'/unitarity.txt','w') as f:
            f.write("\n\n*** Model Layers & Unitariry ***\n\n")
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        ## NOTE mod start
        # add penalty for deviating from desired weight embedding
        if 'pen' in self.args.ver:
            w = model.bert.embeddings.word_embeddings_syno.weight
            distance = torch.linalg.norm(w@w.T-model.bert.embeddings.syno_dis)
            loss_weight = self.args.penalty*distance
            # print(f"loss_orig: {loss:.3f}, loss_weight: {loss_weight:.3f} ({self.args.penalty:.3f}x{distance:.3f})")
            loss += loss_weight
        ## NOTE mod end
    ...
    def evaluation_loop(
        ...
        ## NOTE mod start
        if all_losses is not None:
            metrics[f"loss"] = all_losses.mean().item()
        if 'pen' in self.args.ver:
            with torch.no_grad():
                ...
                loss_weight = self.args.penalty*distance
                metrics[f"loss_weight"] = loss_weight.item()
                metrics[f"loss_total"] = metrics[f"loss"]+loss_weight.item()
                metrics[f"distance"] = distance.item()
                metrics[f"penalty"] = self.args.penalty
        ## NOTE mod end
